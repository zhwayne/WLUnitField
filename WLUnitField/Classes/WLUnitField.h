...
IB_DESIGNABLE
@interface WLUnitField : UIControl <UIKeyInput, UITextInput>
...
@property (null_resettable, nonatomic, strong) IBInspectable UIColor *strokeColor;

/**
 如果需要完成一个 unit 输入后显示地指定已完成的 unit 颜色，可以设置该属性。默认为 nil。
 注意：
    该属性仅在`unitSpace`属性值大于 2 时有效。在连续模式下，不适合颜色跟踪。可以考虑使用`tintColor`替代
 */
...
/**
 是否显示光标，默认为 YES。
 */
@property (nonatomic, assign) IBInspectable BOOL showsCursor;
...
/**
 允许输入的字符集，默认为 nil（允许所有字符）
 设置为数字字符集则只允许数字输入（验证码模式）
 */
